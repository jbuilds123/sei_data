...
async def fetch_and_process_messages(channel):
    ...
    # Define the time threshold (80 hours ago from now)
    time_threshold = datetime.utcnow().replace(
        tzinfo=timezone.utc) - timedelta(hours=80)
        ...
        while True:
            for message in messages:
                if message.date < time_threshold:
                    return processed_messages  # Stop if message is older than threshold
                ...
                if message.text:
                    ...
                    data['created_at'] = int(message.date.replace(
                    ...
                    if links_matches:
                        data['links'] = [{'title': link[0], 'url': link[1]}
                                         for link in links_matches
                                         if "twitter.com" in link[1] or "t.me" in link[1]]

                    processed_messages.append(data)

            # Fetch more messages older than the last one processed
            last_message = messages[-1]
            messages = await client.get_messages(channel, limit=100, max_id=last_message.id)
...
async def main():
    ...
    try:
        ...
        for msg in latest_messages:
            ...
            if pool_address:
                existing_data[pool_address] = msg  # Merge with existing data
