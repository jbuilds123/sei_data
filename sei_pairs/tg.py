...
async def fetch_and_process_messages(channel):
    ...
    min_time_threshold = datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(
        hours=8
    )
    max_time_threshold = datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(
        hours=16
    )
    ...
    async with client:
        ...
        while True:
            for message in messages:
                message_age = (
                    datetime.utcnow().replace(tzinfo=timezone.utc) - message.date
                )

                # Skip if message is newer than 8 hours or older than 16 hours
                if message_age < timedelta(hours=8) or message_age > timedelta(
                    hours=16
                ):
                    continue
            ...
            messages = await client.get_messages(
                channel, limit=100, max_id=last_message.id
            )
            if not messages:
                break
