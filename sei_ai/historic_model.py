...
from sklearn.metrics import precision_score, recall_score, f1_score, roc_auc_score, confusion_matrix, matthews_corrcoef, brier_score_loss, log_loss
...
def focal_loss(gamma=2.0, alpha=0.25):
    def focal_loss_fixed(y_true, y_pred):
        ...
        return -tf.reduce_sum(alpha * tf.pow(1. - pt_1, gamma) * tf.math.log(pt_1)) \
               - tf.reduce_sum((1 - alpha) * tf.pow(pt_0, gamma)
                               * tf.math.log(1. - pt_0))
    return focal_loss_fixed


def compile_and_train_model(model, X_train_sm, y_train_sm, class_weights_dict, save_model_flag=True):
...
# handy usages
probabiliy_threshold = 0.75  # was 0.75
...
calculate_performance_metrics(predictions, pairs_for_predictions, y_for_predictions, sequence_lengths,
                              pair_addresses, end_prices, original_close_prices_sequences, probabiliy_threshold, trade_size)
